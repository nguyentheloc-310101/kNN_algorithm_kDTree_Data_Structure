...
BSTNode<E> *insertBTSNode(BSTNode<E> *root, const E &TKey)
{
    ...
    if (root == nullptr)
        return new BSTNode<E>(TKey);

    //* bước 2.1: xét trường hợp thêm vào cây con bên phải
    if (root->Key < TKey)
    {
        root->right = insertBTSNode(root->right, TKey);
    }
    //* bước 2.2: xét trường hợp thêm vào cây con bên Trái
    else
    {
        root->left = insertBTSNode(root->left, TKey);
    }
